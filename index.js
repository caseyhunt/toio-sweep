...
//0,0 = 0x03, 0x00, 0xA, 0x02, 0xF, 0x00, 0x00,0x57, 0x02, 0xe0, 0x00, 0x67, 0x01
//0, width = 0x03, 0x00, 0xA, 0x00, 0xF, 0x00, 0x00,0x74, 0x01, 0xe0, 0x00, 0x67, 0x01
//height, 0 =
//height, width = 0x03, 0x00, 0xA, 0x02, 0xF, 0x00, 0x00,0x74, 0x01, 0x3c, 0x00, 0xb4, 0x00
...
//min,max in int
movex[2] = [toString(movex[0][0]) + toString(movex[0][1]), toString(movex[1][0]) + toString(movex[1][1])];
let movey = [];
...
for(i=0;i<numSweep;i++){
  let moveby = movey[2][1]-movey[2][0];
  moveby = i*moveby;
  moveby = movey[2][0] + moveby;
  if(moveby<255){
  sweep.push([0x03, 0x00, 0xA, 0x00, 0xF, 0x00, 0x00,movex[0][1], movex[0][0], moveby.toString(16), 0x00, 0x67, 0x01]);
  sweep.push([0x03, 0x00, 0xA, 0x00, 0xF, 0x00, 0x00,movex[1][1], movex[1][0], moveby.toString(16), 0x00, 0x67, 0x01])

}
else{
  //add conversion to little endian bit encoding
  console.log('whoops! looks like you need to write bit encoding for a larger number');
}
